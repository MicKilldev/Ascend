...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_grade'])) {
    ...
    $subject_type = htmlspecialchars($_POST['subject_type']);
    ...
    $stmt->bind_param("isssiddddsss", $student_id, $subject_code, $subject_name, $subject_type, $credits, $prelim, $midterm, $final, $average, $status, $semester, $year_level);
...
$students = $conn->query("
    SELECT u.id, u.username, u.course, s.year_level 
    FROM users u 
...
<body>
        ...
        <form method="POST">
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="course_input" readonly style="background:#f1f5f9;">
                </div>
                <div class="form-group">
                    <label>Year Level</label>
                    <input type="text" name="year_level" id="year_input" placeholder="e.g. 2nd Year" required>
                </div>
            </div>
    ...
    <script>
        document.getElementById('student_select').addEventListener('change', function () {
            ...
            document.getElementById('course_input').value = course ? course : '';
            document.getElementById('year_input').value = year ? year : '';
        });
    </script>
